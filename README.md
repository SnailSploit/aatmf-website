...
This repository ships a static, dependency-free website that surfaces the entire framework — no build step required. Open `index.html` in a browser, or serve the folder:
...
| Page | What it covers |
|:---|:---|
| [`index.html`](index.html) | Overview — hero, 15-tactic index, AATMF-R risk model, standards mapping, audience personas |
| [`tactics.html`](tactics.html) | The 15 tactics across three domains, with per-tactic technique lists and threat signals |
| [`techniques.html`](techniques.html) | 240-technique browser — search, filter by tactic/severity, sort by risk, detail drawer |
| [`detect.html`](detect.html) | Five-layer detection architecture, CaMeL + LlamaFirewall, mitigation controls, alert bands, incident response, YARA/Sigma signatures |
| [`compliance.html`](compliance.html) | OWASP LLM Top 10 2025, OWASP ASI 2026, MITRE ATLAS v4.6.0, NIST AI RMF, EU AI Act — cross-mapped to AATMF |
| [`about.html`](about.html) | Framework thesis, risk model, interoperability, principals, ethical use, citation |

The site is vanilla HTML/CSS/JS (`aatmf.css`, `data.js`, `site.js`, `render-*.js`, `detail.js`) with an optional React-based Tweaks panel (`tweaks-*.jsx`). All technique data is embedded in `data.js`.
